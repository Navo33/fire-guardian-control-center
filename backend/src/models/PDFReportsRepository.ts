...
export class PDFReportsRepository {
  ...
  /**
   * Get comprehensive report data for a vendor's client within a date range
   * Used by vendors to generate reports for their clients
   */
  async getVendorClientReportData(
    vendorId: number,
    clientId: number,
    startDate: string,
    endDate: string
    ...
    try {
      // 1. Get client info and verify it belongs to this vendor
      const clientResult = await pool.query(
        `SELECT 
          c.id,
          c.company_name,
        ...
        FROM clients c
        JOIN "user" u ON c.user_id = u.id
        WHERE c.id = $1 AND c.created_by_vendor_id = $2 AND c.status = 'active'`,
        [clientId, vendorId]
      );
      ...
      // 3. Get equipment summary for this client
      const summaryResult = await pool.query(
        `SELECT 
          COUNT(*) as total_equipment,
          COUNT(*) FILTER (WHERE compliance_status = 'compliant') as compliant_equipment,
          ...
          COUNT(*) FILTER (WHERE compliance_status = 'overdue' OR 
            (next_maintenance_date IS NOT NULL AND next_maintenance_date < CURRENT_DATE)) as maintenance_due
        FROM equipment_instance
        WHERE assigned_to = $1 AND vendor_id = $2 AND deleted_at IS NULL`,
        [clientId, vendorId]
      );

      const summary = summaryResult.rows[0];

      // 4. Get equipment details for this client
      const equipmentResult = await pool.query(
        `SELECT 
          e.equipment_name,
          e.equipment_type,
      ...
      const maintenanceResult = await pool.query(
        ...
        WHERE mt.client_id = $1
          AND mt.vendor_id = $2
          AND mt.created_at BETWEEN $3 AND $4
        ORDER BY mt.created_at DESC`,
      ...
      // Generate report number
      const reportNumber = `FG-${vendorId}-${clientId}-${new Date().getFullYear()}${String(new Date().getMonth() + 1).padStart(2, '0')}${String(new Date().getDate()).padStart(2, '0')}-${Math.floor(Math.random() * 1000).toString().padStart(3, '0')}`;
      ...
      console.error('Error fetching vendor client report data:', error);
      throw error;
    }
  }

  /**
   * Get list of clients for a vendor (for report filtering)
   */
  async getVendorClients(vendorId: number): Promise<Array<{ id: number; companyName: string }>> {
    try {
   ...
   */
  async getVendorIdFromUserId(userId: number): Promise<number | null> {
    try {
      const result = await pool.query(
