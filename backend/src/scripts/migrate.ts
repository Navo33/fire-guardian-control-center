...
/**
 * Load all SQL migration files from migrations folder
 */
export const loadSqlMigrations = (): Migration[] => {
  ...
  return files.map(file => {
    ...
    return {
      name: migrationName,
      description: `Execute migration: ${migrationName}`,
      up: async () => {
        console.log(`📝 Executing SQL migration: ${migrationName}...`);
...
export const runMigrations = async (): Promise<void> => {
  ...
  // Build migrations array: initial schema + all SQL migrations from migrations folder
  const sqlMigrations = loadSqlMigrations();
  ...
  const migrations: Migration[] = [
    ...
    ...sqlMigrations  // Load all SQL migrations from migrations folder dynamically
  ];
